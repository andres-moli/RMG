...
import { OrderStatusEnum } from "src/main/rmg/orderRepair/enum/emunOrderRepair";
import { FieldTypeEnum } from "src/main/rmg/repairType/emun/FieldTypeEnum";

export const DOCUMENT_ENTRY_PRINT = (repair: OrderRepairty) => {
    dayjs.locale("es");
    ...
    const fieldValuesHtml = repair?.fieldValues?.map((fieldValue) => {
      ...
      switch (fieldValue.field.type) {
          ...
          case FieldTypeEnum.IMAGE:
              fieldContent = fieldValue.valorFoto?.fileUrl ? `<img src="${fieldValue.valorFoto.fileUrl}" alt="Imagen">` : "<span>N/A</span>";
              break;
          default:
  ...
  <body>
      ${repair?.status == OrderStatusEnum.CANCELED ? '<div class="sello-anulado">REPRACION CANCELADA</div>' : ''}
